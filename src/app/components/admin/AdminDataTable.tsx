          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '620px' }}>
            <thead>
              <tr style={{ backgroundColor: '#F8F8F8', borderBottom: '1px solid rgba(0,0,0,0.07)' }}>
                {columns.map(col => (
                  <th key={col.key} style={{ padding: '12px 16px', textAlign: 'left', fontSize: '11px', fontWeight: 700, color: '#5A7A5A', textTransform: 'uppercase', letterSpacing: '0.07em', whiteSpace: 'nowrap' }}>
                    {col.header}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {data.length === 0 ? (
                <tr>
                  <td colSpan={columns.length} style={{ padding: '48px', textAlign: 'center', color: '#5A7A5A', fontSize: '14px' }}>
                    {emptyMessage}
                  </td>
                </tr>
              ) : data.map((item, idx) => {
                ...
                if (isDeleting) {
                  return (
                    <tr key={`delete-${baseKey}`} style={{ backgroundColor: '#FEF2F2' }}>
                      <td colSpan={columns.length} style={{ padding: '14px 16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
                          <AlertTriangle size={15} style={{ color: '#DC2626', flexShrink: 0 }} />
                          <span style={{ fontSize: '14px', color: '#1A1A1A', flex: 1 }}>
                            ¿Eliminar <strong>"{String(item.title ?? '')}"</strong>? Esta acción no se puede deshacer.
                          </span>
                          <div style={{ display: 'flex', gap: '8px' }}>
                            <button onClick={onDeleteCancelled} style={{ padding: '6px 14px', borderRadius: '6px', border: '1px solid rgba(0,0,0,0.14)', backgroundColor: '#FFFFFF', color: '#1A1A1A', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>
                              Cancelar
                            </button>
